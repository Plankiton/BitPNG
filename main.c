...
int main(int c, char **a) {
  ...
  while ((idat = next_chunk(image)) &&
         strcmp((const char *)idat->type, "IDAT")) {
    // fwrite_chunk(outfile, idat);

    // Como o chunk não é um IDAT ele deve ser limpo antes de buscarmos o
    // próximo
    trash_chunk(idat);
  }

  // Enquanto o chunk não for nulo e diferente de IEND (ou seja, enquanto o
  // chunk é um IDAT)
  int i = 0;
  do {
    ...
    for (int j = 0; j + 2 < idat->lenght; j += 3) {
      ...
      Byte average = (data[j + 0] + data[j + 1] + data[j + 2]) / 3;
      // printf("%i: %i %i %i -> %i\n", j, data[j], data[j + 1], data[j + 2],
      //       average);
      data[j + 0] = 0xFF;
      // data[j + 1] = 0;
      // data[j + 2] = 0;
    ...
    trash_chunk(idat);
           ...
           strcmp((const char *)idat->type, "IEND"));
